...
class ThreeGeo {
    ...
    static getUriMapbox(token, api, zoompos) {
        ...
        // return `https://b.tile.openstreetmap.org/${zoompos.join('/')}.png`
    }
    ...
    processRgbTile(pixels, zoomposEle, zpCovered, bbox, radius) {
        ...
        sixteenths.forEach((zoomposStr, index) => {
            ...
            for (let row = 0; row < constVertices; row++) {
                ...
                if (row == 0 || row == constVertices - 1) {
                    for (let col = 0; col < constVertices; col++) {
                        ...
                        let px_py = this._projectCoord(lonlatPixel, bbox.northWest, bbox.southEast)
                        let pz = elev[dataIndex] * unitsPerMeter;
                        px_py.push(pz)
                        borderArray.push(px_py)
            ...
            demTileBorder.border.push(borderArray) //存储dem数据瓦片的边界像素的坐标
            // console.log('zoompos, array:', zoompos, array); // 49152 = 128*128*3 elements
            dataEle.push([zoompos, array, zoomposEle, demTileBorder]);
            //viewer.demTileBorder.push(borderArray);
        });
    ...
    _getRgbDem(dataEle, apiSatellite, _onSatelliteMat) {
        ...
        let hhh = dataEle.length
        window.dataEle = dataEle
        dataEle.forEach(([zoompos, arr, zoomposEle, demTileBorder], index) => {
            ...
            // if (index == 0) {
            //     demTileBorder[demBoxLeftTop] = [arr[0], arr[1]]
            // }
            // if (index == hhh - 1) {
            //     demTileBorder[demBoxRightBottom] = [arr[49149], arr[49150]]
            // }
    ...
    _getRgbTiles(zpCovered, bbox, radius, apiRgb, apiSatellite,
        onRgbDem, onSatelliteMat, watcher, onDemTileBorder, onGeologyLayer) {
            ...
            try {
                ...
                if (cbs.onRgbDem) {
                    this._getRgbTiles(zpCovered, bbox, radius,
                        ...
                        cbs.onRgbDem, cbs.onSatelliteMat, watcher, cbs.onDemTileBorder, cbs.onGeologyLayer);
                }
            } catch (err) {
